...
class App(customtkinter.CTk):
    #
    # This app is made by Eshaan Kapooswalla
    #
    ...
    etfsValue = sum(entry[2] for entry in currentETFs.values())  # Corrected to currentETFs
    cryptoValue = sum(entry[2] for entry in currentCrypto.values())  # Corrected to currentCrypto
    ...
    def __init__(self):
        super().__init__()
        self.title("StockPredictor")  # Changed the title here
        ...
        title = customtkinter.CTkLabel(
            master=self,
            text="StockPredictor",  # Changed title in the UI as well
        ...
        addFundsSubLabel = customtkinter.CTkLabel(
            master=settingsFrame,
            text="Want to feel like a millionaire? Lost all your money already? No problem, add more funds here.",
            font=customtkinter.CTkFont(size=15),
        )
